...
import { addClientDocumentFormSchema } from "@utils/schemas";
...
import { useClientDocumentsStore } from "@stores/clients/documents";
...
const documentsStore = useClientDocumentsStore();
const { addDocument } = documentsStore;

const { handleSubmit, isSubmitting } = useForm({
  validationSchema: addClientDocumentFormSchema,
...
const onSubmit = handleSubmit(
  async (values) => {
    const { title } = values;
    const success = await addDocument(clientId, title, uploadedFile.value);
