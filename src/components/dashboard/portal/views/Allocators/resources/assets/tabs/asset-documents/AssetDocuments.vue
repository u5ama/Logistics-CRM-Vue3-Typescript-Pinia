...
const assetStore = useAssetDocumentsStore();
...
const deleteDocHandler = async (docId: number) => {
  ...
  useToast().success("Asset Document deleted Successfully!");
  setDeleting.stop();
};
</script>
...
<template>
  <div class="wrapper max-w-6xl mt-6 mx-auto">
    ...
    <Panel>
      <template #header>
        <StyledHeading level="h3">Asset Documents</StyledHeading>
      </template>
      <template #icons>
        <Button
          icon="pi pi-plus"
