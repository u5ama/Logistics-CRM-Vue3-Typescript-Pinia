...
import { ref } from "vue";
import CreateExistingInvoices from "@dashboard-views/jobs/edit-job/tabs/job-timesheets/CreateExistingInvoices.vue";

const timeSheetStore = useJobTimeSheetStore();
const { timesheets } = storeToRefs(timeSheetStore);
const { getAllTimeSheets, deleteTimeSheet, createInvoiceForMultiple } =
  timeSheetStore;
const [isLoading, setLoading] = useLoading();
const [isModalOpen, setModal] = useModal();
const [isModalInvoiceOpen, setInvoiceModal] = useModal();
...
const createInvoice = async () => {
  await createInvoiceForMultiple(selectedProduct.value);
...
<template>
  <div class="wrapper max-w-6xl mt-6 mx-auto">
    <CreateExistingInvoices
      v-if="isModalInvoiceOpen"
      :open="isModalInvoiceOpen"
      @close="setInvoiceModal.close"
    ...
    />
    <div class="flex mb-2 justify-end" v-if="selectedProduct.length > 0">
      <button
        class="p-button p-button-primary mb-5 ml-3"
        @click="setInvoiceModal.open"
      >
    ...
    <Panel>
      ...
      <div class="flex flex-col gap-y-6" v-else-if="timesheets.length">
        ...
        >
          ...
          <Column field="invoice_status" class="invoice_status" header="Invoice Status"></Column>
