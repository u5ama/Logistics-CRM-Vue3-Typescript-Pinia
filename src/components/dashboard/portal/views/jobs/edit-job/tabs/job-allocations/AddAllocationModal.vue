...
const { allDrivers, jobAssets } = storeToRefs(driversStore);
const { addDriver, allDriversList, getAllJobAssets } = driversStore;
...
const onSubmit = handleSubmit(
  async (values) => {
    const { driver_id, asset_id } = values;
    const success = await addDriver(jobId, driver_id, asset_id);

    if (!success) return;
    useToast().success("Asset allocated Successfully!");
    emit("close");
  },
    ...
    <Transition name="modal">
      <div v-if="open" class="modal-mask">
        ...
        >
          ...
          <StyledHeading class="mb-12">
            Allocate New Fieldworker
          </StyledHeading>
