...
import AdminDriverJobsTable from "@dashboard/widgets/drivers/AdminDriverJobsTable.vue";
import { useRoute } from "vue-router";

const driverJobsStore = useDriverJobsStore();
const { allocatedJobs } = storeToRefs(driverJobsStore);
const { getAllAllocatedJobs } = driverJobsStore;
...
const fetchData = async () => {
  setLoading.start();
  await getAllAllocatedJobs(driverId);
  setLoading.stop();
};
