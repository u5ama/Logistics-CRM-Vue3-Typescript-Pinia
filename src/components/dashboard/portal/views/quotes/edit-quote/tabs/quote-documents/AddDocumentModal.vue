...
import { addSupplierDocumentFormSchema } from "@utils/schemas";
...
import { useQuoteDocumentsStore } from "@stores/quotes/documents";
...
defineProps<Props>();
const { quoteId } = inject(editQuoteInjectionKey)!;
const quoteDocumentsStore = useQuoteDocumentsStore();
const { addDocument } = quoteDocumentsStore;

const { handleSubmit, isSubmitting } = useForm({
  validationSchema: addSupplierDocumentFormSchema,
...
const onSubmit = handleSubmit(
  async (values) => {
    const { title } = values;
    const success = await addDocument(quoteId, title, uploadedFile.value);
    ...
    <Transition name="modal">
      <div v-if="open" class="modal-mask">
        ...
        >
          ...
          <StyledHeading class="mb-12"> Add New Document </StyledHeading>
